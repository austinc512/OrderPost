...
DROP TABLE IF EXISTS clonestation_ship_to;
DROP TABLE IF EXISTS clonestation_ship_from;
DROP TABLE IF EXISTS clonestation_order_items;
DROP TABLE IF EXISTS clonestation_products;
DROP TABLE IF EXISTS clonestation_orders;
DROP TABLE IF EXISTS clonestation_shipments;
DROP TABLE IF EXISTS clonestation_customers;

CREATE TABLE clonestation_customers (
...
CREATE TABLE clonestation_ship_to (
    id INT PRIMARY KEY auto_increment,
    customer_id INT,
    first_name VARCHAR (40),
    last_name VARCHAR (40),
    ...
    REFERENCES clonestation_customers (id)
    ON DELETE CASCADE
);

CREATE TABLE clonestation_ship_from (
    id INT PRIMARY KEY auto_increment,
    first_name VARCHAR (40),
    last_name VARCHAR (40),
...
CREATE TABLE clonestation_orders (
    ...
    ship_from_id INT,
    FOREIGN KEY (ship_from_id)
    REFERENCES clonestation_ship_from (id)
    ON DELETE CASCADE,
...
CREATE TABLE clonestation_products (
...
CREATE TABLE clonestation_order_items (
    ...
    REFERENCES clonestation_orders (order_id)
    ON DELETE CASCADE,
    FOREIGN KEY (product_id)
    REFERENCES 
    clonestation_products (product_id)
    ON DELETE CASCADE
);

-- once a shipping label is created, we can insert into Shipments table
CREATE TABLE clonestation_shipments (
